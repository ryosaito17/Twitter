...
public abstract class BsUserBhv extends AbstractBehaviorWritable<User, UserCB> {
     ...
     * @throws SelectEntityConditionNotFoundException When the condition for selecting an entity is not found.
     */
    public OptionalEntity<User> selectByPK(Long userId) {
        return facadeSelectByPK(userId);
    }

    protected OptionalEntity<User> facadeSelectByPK(Long userId) {
        return doSelectOptionalByPK(userId, typeOfSelectedEntity());
    }

    protected <ENTITY extends User> ENTITY doSelectByPK(Long userId, Class<? extends ENTITY> tp) {
        return doSelectEntity(xprepareCBAsPK(userId), tp);
    }

    protected <ENTITY extends User> OptionalEntity<ENTITY> doSelectOptionalByPK(Long userId, Class<? extends ENTITY> tp) {
        return createOptionalEntity(doSelectByPK(userId, tp), userId);
    }

    protected UserCB xprepareCBAsPK(Long userId) {
     ...
     * @throws SelectEntityConditionNotFoundException When the condition for selecting an entity is not found.
     */
    public OptionalEntity<User> selectByUniqueOf(String accountId) {
        return facadeSelectByUniqueOf(accountId);
    }

    protected OptionalEntity<User> facadeSelectByUniqueOf(String accountId) {
        return doSelectByUniqueOf(accountId, typeOfSelectedEntity());
    }

    protected <ENTITY extends User> OptionalEntity<ENTITY> doSelectByUniqueOf(String accountId, Class<? extends ENTITY> tp) {
        return createOptionalEntity(doSelectEntity(xprepareCBAsUniqueOf(accountId), tp), accountId);
    }

    protected UserCB xprepareCBAsUniqueOf(String accountId) {
        assertObjectNotNull("accountId", accountId);
        return newConditionBean().acceptUniqueOf(accountId);
    }
     ...
     * <pre>
     * <span style="color: #0000C0">userBhv</span>.<span style="color: #CC4747">loadGood</span>(<span style="color: #553000">userList</span>, <span style="color: #553000">goodCB</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     ...
     */
    public NestedReferrerListGateway<Good> loadGood(List<User> userList, ReferrerConditionSetupper<GoodCB> refCBLambda) {
        xassLRArg(userList, refCBLambda);
        return doLoadGood(userList, new LoadReferrerOption<GoodCB, Good>().xinit(refCBLambda));
    }
     ...
     * GOOD by USER_ID, named 'goodList'.
     * <pre>
     * <span style="color: #0000C0">userBhv</span>.<span style="color: #CC4747">loadGood</span>(<span style="color: #553000">user</span>, <span style="color: #553000">goodCB</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     <span style="color: #553000">goodCB</span>.setupSelect...
     *     <span style="color: #553000">goodCB</span>.query().set...
     *     <span style="color: #553000">goodCB</span>.query().addOrderBy...
     * }); <span style="color: #3F7E5E">// you can load nested referrer from here</span>
     ...
     * <span style="color: #3F7E5E">//    ...</span>
     * <span style="color: #3F7E5E">//});</span>
     * ... = <span style="color: #553000">user</span>.<span style="color: #CC4747">getGoodList()</span>;
     * </pre>
     * About internal policy, the value of primary key (and others too) is treated as case-insensitive. <br>
     * The condition-bean, which the set-upper provides, has settings before callback as follows:
     * <pre>
     ...
     * @return The callback interface which you can load nested referrer by calling withNestedReferrer(). (NotNull)
     */
    public NestedReferrerListGateway<Good> loadGood(User user, ReferrerConditionSetupper<GoodCB> refCBLambda) {
        xassLRArg(user, refCBLambda);
        return doLoadGood(xnewLRLs(user), new LoadReferrerOption<GoodCB, Good>().xinit(refCBLambda));
    }

    protected NestedReferrerListGateway<Good> doLoadGood(List<User> userList, LoadReferrerOption<GoodCB, Good> option) {
        return helpLoadReferrerInternally(userList, option, "goodList");
    }
    ...
    //                                                                   =================
    /**
     * Pull out the list of referrer-as-one table 'Login'.
     * @param userList The list of user. (NotNull, EmptyAllowed)
     * @return The list of referrer-as-one table. (NotNull, EmptyAllowed, NotNullElement)
     */
    public List<Login> pulloutLoginAsOne(List<User> userList)
    { return helpPulloutInternally(userList, "loginAsOne"); }

    /**
     * Pull out the list of referrer-as-one table 'Mutter'.
     * @param userList The list of user. (NotNull, EmptyAllowed)
     * @return The list of referrer-as-one table. (NotNull, EmptyAllowed, NotNullElement)
     */
    public List<Mutter> pulloutMutterAsOne(List<User> userList)
    { return helpPulloutInternally(userList, "mutterAsOne"); }

    /**
     * Pull out the list of referrer-as-one table 'Relation'.
     * @param userList The list of user. (NotNull, EmptyAllowed)
     * @return The list of referrer-as-one table. (NotNull, EmptyAllowed, NotNullElement)
     */
    public List<Relation> pulloutRelationAsOne(List<User> userList)
    { return helpPulloutInternally(userList, "relationAsOne"); }
     ...
     */
    public List<Long> extractUserIdList(List<User> userList)
    { return helpExtractListInternally(userList, "userId"); }

    /**
     * Extract the value list of (single) unique key accountId.
     * @param userList The list of user. (NotNull, EmptyAllowed)
     * @return The list of the column value. (NotNull, EmptyAllowed, NotNullElement)
     */
    public List<String> extractAccountIdList(List<User> userList)
    { return helpExtractListInternally(userList, "accountId"); }
